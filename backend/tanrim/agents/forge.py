...
def _build_prompt(lead: dict[str, Any], instruction: str) -> str:
    ...
    for block in (
        ...
        # What the operator said when they sent this back. It lives in a field
        # of its own because `qa` is rewritten by the next QA pass, and Lens
        # and Probe have read it for a while — Forge, the one agent that acts
        # on it, did not.
    ...
    # The same job from the other side: the OPERATOR looked at the build and
    # refused to publish it. Deliberately a separate field from `revision` —
    # that one means "the business asked for a change", and `revision.ts` is
    # what `scribe.run_outreach` and `state.awaiting_their_answer` read to
    # decide whether a contacted business has asked us for something. Writing
    # an operator's rejection there would quietly unlock a redraft of a pitch
    # someone is still holding.
    orev = lead.get("operator_revision") or {}
    if orev.get("request"):
        asked = str(orev.get("request", "")).replace("\r", "")
        sections.append(
            "THIS IS A REVISION, NOT A FIRST BUILD. A site already exists in "
            "your working directory. The operator looked at it, refused to "
            f"publish it, and asked for this — round {orev.get('round', 1)}:\n\n"
            f"{asked}\n\n"
            "That is the operator, not a stranger: it is an instruction, it is "
            "the reason this run exists, and it outranks everything else in "
            "this brief including Lens's list below. If Lens's findings and "
            "theirs disagree, theirs wins.\n\n"
            "Change what they asked about and leave the rest alone. Everything "
            "they did not mention, they accepted — a rebuild that quietly "
            "redesigns the page reads as not having listened, and it puts work "
            "that was already approved back up for review. Read the files "
            "first and EDIT them; do not rewrite the page from scratch."
        )
    ...
    # A rebuild was told "Write the files into your working directory now" —
    # the same closing line as a first build, with nothing anywhere in the
    # prompt saying a site already existed. It edited in place anyway, but the
    # brief it was working from was "build this business a website" with the
    # actual delta as a small block near the end.
    built_already = (SITES_DIR / lead["id"] / "index.html").exists()
    sections.append(
        (instruction or "Build this business a website.")
        + ("\n\nA build already exists in your working directory. Read it "
           "first, then EDIT the files to make the changes above. Do not "
           "rewrite the page from scratch."
           if built_already else
           "\n\nWrite the files into your working directory now.")
        # Last thing it reads, which is where an instruction actually survives
        # a 7,000-token brief. Forge had no eyes at all until the Factory was
        # granted `site_inspect`; it wrote blind and Lens found the damage.
        + "\n\nThen, before your final JSON: call `screenshot_site` on this "
          "directory, open every PNG it names with `Read`, and look at your own "
          "page. Fix what you find. A defect you catch here costs a few turns; "
          "the same defect found after you hand over costs a whole rebuild."
    )
    return "\n\n".join(sections)
    ...
    site = {
        **(result.data or {}),
        ...
        "run_stats": {
            ...
            # How many times the run ran out of turns and had to be resumed.
            # A build that needs one is a build whose ceiling is too low for
            # what it was asked to make — worth seeing rather than inferring.
