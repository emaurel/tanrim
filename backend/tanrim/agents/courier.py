...
from .. import config
from .. import domains, hosting, sandbox, state
...
def _preview_banner(business: str, kind: str = "prospect") -> str:
    """The stamp that says what this page is.

    A PROSPECT has not asked for anything: the banner exists so a stranger who
    opens the link cannot mistake an unsolicited mock-up for the business's own
    site, and it says plainly that we are not affiliated with them.

    A PORT client commissioned the rebuild. "Not affiliated with or endorsed by"
    is then simply false — they endorsed it by asking — and reads as a disclaimer
    on work they are paying for. What is still true, and still worth saying, is
    that this is a preview and not yet their live site.
    """
    who = config.AGENCY_NAME or "an independent web designer"
    if kind == "port":
        return (
            f'<div style="{BANNER_CSS}">Aperçu — nouvelle version du site de '
            f'{business}, en préparation par {who}. Ce n\'est pas encore le '
            f'site en ligne.</div>'
        )
    return (
...
def _prepare(html: str, business: str, kind: str = "prospect") -> str:
    """Stamp the page as a preview before it is ever reachable."""
    if "robots" not in html.lower():
        html = re.sub(r"(<head[^>]*>)", r"\1\n  " + NOINDEX, html, count=1, flags=re.I)
    if "Unofficial preview" not in html and "Aperçu —" not in html:
        html = re.sub(r"(</body>)", _preview_banner(business, kind) + r"\1", html,
                      count=1, flags=re.I)
...
async def request_publish(world: World, lead_id: str) -> dict[str, Any]:
    ...
    # A made-up business must not reach a public URL where it could be taken
    # for a real one. `/staging/<id>/` serves the build off disk instead.
    if sandbox.is_sandbox(lead):
        return {"ok": False, "error": sandbox.REFUSAL,
            ...
            prepared = {
                path: (
                    _prepare(content.decode("utf-8", "replace"),
                             lead.get("name", "this business"),
                             state.lead_kind(lead)).encode("utf-8")
