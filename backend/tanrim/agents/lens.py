...
async def run_qa(world: World, lead_id: str, instruction: str = "") -> dict[str, Any]:
    ...
    # Carry the operator's own words forward. `qa` is replaced wholesale on
    # every QA pass, so a rejection reason that lived only here survived
    # exactly one rebuild: Lens re-QA'd, wrote a fresh dict, and the thing that
    # caused the rebuild was gone before the second attempt — which is how two
    # leads in one evening got three requests answered once and then never
    # mentioned again. Dropped only when the operator approves the publish.
    problems = list(parsed.get("problems") or [])
    # Only while the build is still FAILING. On a pass the lead goes straight
    # back to the operator, and listing their own already-fixed complaint as an
    # outstanding critical on the publish card invites them to reject a build
    # that is actually done. Nothing is lost by dropping it here: `sent_back`
    # and `operator_revision` hold the same words until the publish is
    # approved, and those are what Forge reads.
    carried = [p for p in ((lead.get("qa") or {}).get("problems") or [])
               if p.get("where") == "operator"] if verdict != "pass" else []
    if carried:
        seen = {json.dumps(p, sort_keys=True, ensure_ascii=False) for p in problems}
        for p in reversed(carried):
            if json.dumps(p, sort_keys=True, ensure_ascii=False) not in seen:
                problems.insert(0, p)
    qa = {**parsed, "problems": problems,
          "cost_usd": result.cost_usd, "rounds": rounds}
...
def _build_visual_prompt(lead: dict[str, Any], out_dir: str) -> str:
    ...
    # `sources` is contracted as [{"url": ..., "gave_us": ...}], but a model
    # writes whatever the schema in front of it asked for — and the port
    # survey's schema asked for bare strings, which crashed this line on the
    # first real port lead. A prompt can only ASK for a shape; the code that
    # reads it should not fall over when it gets the other reasonable one.
    directory = []
    for src in (profile.get("sources") or []):
        url = src.get("url") if isinstance(src, dict) else src
        if isinstance(url, str) and url.strip():
            directory.append(url.strip())
    if lead.get("website"):
