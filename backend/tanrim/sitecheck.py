...
def check_photo_use(site_dir: Path, pages: list[tuple[str, str]]) -> list[Problem]:
    """Did the build use the photographs it was given?

    A real build produced a page referencing nothing but `logo.svg` while 21
    usable photographs of the business sat in `photos/`. Nothing caught it
    until the operator did, and by then a whole rebuild was needed.

    Photographs of the actual place are the single biggest thing separating one
    of these pages from a template, so a build that ignores all of them is
    making a large claim — that not one was usable — and should have to say so
    rather than do it silently.
    """
    photo_dir = site_dir / "photos"
    if not photo_dir.is_dir():
        return []
    available = [f.name for f in sorted(photo_dir.iterdir())
                 if f.is_file() and f.suffix.lower() in
                 {".jpg", ".jpeg", ".png", ".webp"}
                 and "-" not in f.stem.rsplit("-", 1)[-1].rstrip("0123456789")]
    # Variants generated by images.responsive are not candidates.
    available = [n for n in available
                 if not re.search(r"-\d{3,4}\.webp$", n)]
    if not available:
        return []
    used = set()
    for _, html in pages:
        used |= set(re.findall(r"photos/([A-Za-z0-9._\-]+)", html))
    if used:
        return []
    return [{
        "severity": "major", "page": "site",
        "problem": f"{len(available)} photographs of this business were "
                   "harvested and the page uses none of them. A page with real "
                   "pictures of the place is the main thing separating this "
                   "from a template — if genuinely none were usable, say which "
                   "and why in `placeholders`.",
    }]


#: An actual dark-mode at-rule, not the words in a comment explaining its
#: absence. `@media` and the feature may be separated by other conditions.
_RULE_RE = re.compile(r"@media[^{]*prefers-color-scheme\s*:\s*dark", re.I)


def check_theme(site_dir: Path, css: str) -> list[Problem]:
    """An asset that changes colour with the OS theme, on a page that doesn't.

    Forge draws the wordmark as an SVG with its own stylesheet, and it likes to
    write a dark-mode rule into it:

        .mark { fill: #1A1A1A; }
        @media (prefers-color-scheme: dark) { .mark { fill: #F2EFEA; } }

    That is correct on a page that also has a dark theme. On these pages it is
    a disaster, because the bar the mark sits on is a hardcoded cream and the
    stylesheet has no `prefers-color-scheme` rule anywhere: every visitor whose
    browser is in dark mode — which is most phones after sunset — gets a
    near-white wordmark on a near-white bar and cannot see the business's name
    at all.

    It went unnoticed through four rebuilds of one site and shipped live on two
    others, because nothing in the pipeline renders in dark mode: Playwright
    defaults to `prefers-color-scheme: light`, so every screenshot the QA agent
    has ever judged was a light-mode render. The operator reported it three
    times as "the title is white on white" and each rebuild fixed a different
    element.

    Static, because it is a fact rather than a judgement: either the page
    answers the dark-mode query or its assets must not ask it.
    """
    def _live(text: str) -> str:
        """The markup with comments removed.

        Both halves of this check have to ignore comments, and the first
        version did not: two builds had DELETED their dark-mode rule and left a
        comment saying why — "a prefers-color-scheme:dark rule would turn RBS
        white-on-white" — and got flagged for the bug they had already fixed.
        A rule is an at-rule, not a mention.
        """
        text = re.sub(r"<!--.*?-->", " ", text, flags=re.S)
        return re.sub(r"/\*.*?\*/", " ", text, flags=re.S)

    if _RULE_RE.search(_live(css)):
        return []
    offenders = []
    for svg in sorted(site_dir.glob("*.svg")):
        try:
            if _RULE_RE.search(_live(svg.read_text(encoding="utf-8", errors="replace"))):
    ...
    if not offenders:
        return []
    return [{
        "severity": "critical", "page": "site",
        "problem": f"{', '.join(offenders)} switch{'es' if len(offenders) == 1 else ''} "
                   "colour under `@media (prefers-color-scheme: dark)`, but the "
                   "stylesheet has no dark-mode rule at all — the surfaces stay "
                   "light. For every visitor whose browser is in dark mode the "
                   "mark turns light-on-light and the business's name is "
                   "invisible. Either delete the dark-mode block from the SVG "
                   "(the surface it sits on never changes), or give the page a "
                   "real dark theme. Deleting it is almost always right.",
    }]
