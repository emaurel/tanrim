...
async function render({ roomId, data, body, reload }: PanelContext) {
  ...
  if (un?.rows) {
    ...
    p.textContent =
      ...
      + "attributed to any lead — nothing recorded the association before that "
      + "was added, so the totals above are complete but the per-lead figures on "
      + "each lead's card start from then.";
    body.appendChild(p);
  }
